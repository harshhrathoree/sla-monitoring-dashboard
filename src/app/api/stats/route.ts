 ...
 * GET /api/stats
 *
 * Returns per-service SLA statistics aggregated over all monitoring_checks in
 * the database, plus overall uptime and a per-day breakdown for each service.
 *
 * This is a read-only query route — no mutations.
 *
 * Response shape: StatsResponse (see src/types/api.ts)
 */
...
// ─── Raw Postgres row shapes (cast from db.execute results) ───────────────────
...
/**
 * Groups chronologically-sorted error timestamps for one service into incident
 * windows. Two consecutive checks belong to the same incident if the gap
 * between them is ≤ INCIDENT_GAP_MINUTES.
 */
...
export async function GET() {
  try {
    ...
    // ── 1. Per-service aggregates ─────────────────────────────────────────
    // PERCENTILE_CONT is a Postgres ordered-set aggregate — not available in
    // Drizzle's query builder, so we use raw sql`` template literals.
    ...
    // ── 2. Per-service, per-day breakdown ─────────────────────────────────
    ...
    // ── 3. Error rows per service (for incident detection) ────────────────
    ...
    const response: StatsResponse = {
      ...
      overallUptimePct,
      lastUploadAt:
        uploadRows.length > 0 ? new Date(uploadRows[0].uploaded_at).toISOString() : null,
